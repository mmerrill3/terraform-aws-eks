...
locals {
  launch_template_name = coalesce(var.launch_template_name, "${var.name}${var.prefix_separator}eks-node-group")
...
locals {
  iam_role_name          = coalesce(var.iam_role_name, "${var.name}${var.prefix_separator}eks-node-group")
